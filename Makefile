.PHONY: all build build_linux clean run check cover lint docker help
BIN_FILE=image
...
build:
	@go build -o "${BIN_FILE}"
build_linux:
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o "${BIN_FILE}_linux"
clean:
	@go clean
	rm --force "xx.out"
test:
...
docker:
	@docker build -t sre/tinker:latest .
help:
